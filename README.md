...
3. **Jev** (when enabled for the source): one request per event carries 6 questions (see `questions.py`, version `ops-notification-v1`). The response must include every answer, a complete probability distribution whose highest option matches the chosen category, and the requested pinned model (unless `allow_model_alias = true`). Invalid responses take the non-dropping fallback route. The thresholds are applied in `policy.route_from_answers`, and per-source overrides come from `[sources.<name>].thresholds`; unknown category names are rejected at config load.
   After the notify checks, uncertain `security`/`data_integrity` categories go to `REVIEW` rather than `DROP`; otherwise the DROP checks apply, then harm ≥ `review_harm` (default `0.40`) goes to `REVIEW`. All other unmatched verdicts go straight to `DIGEST`. Thus Jev decides whether a webhook agent must run per event. Per-source `review_harm` overrides are supported.
...
`deliver: log` only suppresses Telegram delivery; it does **not** prevent a Hermes model call. Only `DIGEST` and `DROP` decisions avoid per-event Hermes runs. In guarded mode, classifier `DROP` is still queued for the digest rather than discarded, and digest summarization can call Hermes on a batch. Avoid switching to `full` solely for cost unless the labeled-corpus DROP guardrail passes.
